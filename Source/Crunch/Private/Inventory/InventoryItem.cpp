...
void UInventoryItem::RemoveGASModifications(UAbilitySystemComponent* AbilitySystemComponent)
{
	...
	{
		AbilitySystemComponent->RemoveActiveGameplayEffect(AppliedEquipEffectHandle);
	}
...
void UInventoryItem::ApplyGASModifications(UAbilitySystemComponent* AbilitySystemComponent)
{
	...
	if (EquipEffect)
	{
		AppliedEquipEffectHandle = AbilitySystemComponent->BP_ApplyGameplayEffectToSelf(EquipEffect,1,AbilitySystemComponent->MakeEffectContext());
	}
