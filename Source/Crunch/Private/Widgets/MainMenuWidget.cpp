...
void UMainMenuWidget::JoinSessionBtnClicked()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Try to join session with id:%s"),*CurrentSelectedSessionId);
...
{
	UE_LOG(LogTemp,Warning,TEXT("LoginBtnClicked"));
...
void UMainMenuWidget::LoginCompleted(bool bWasSuccessful, const FString& PlayerNickname, const FString& ErrorMsg)
{
	if (bWasSuccessful)
	{
		UE_LOG(LogTemp,Warning,TEXT("LoginCompleted"));
...
{
	UE_LOG(LogTemp,Warning,TEXT("Update Lobby Session Search Results"));
