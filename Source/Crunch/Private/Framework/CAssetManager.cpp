...
UCAssetManager& UCAssetManager::Get()
{
	...
	UE_LOG(LogLoad,Fatal,TEXT("UCAssetManager::AddItem"));
	...
	if (GetLoadedShopItems(LoadedItems))
	{
		for (const UPA_ShopItem* Item : LoadedItems)
		{
			...
			for (const TSoftObjectPtr<UPA_ShopItem>& Ingredient : Item->GetIngredients())
			{
				...
				AddToCombination(IngredientItem,Item);			
			}
...
void UCAssetManager::AddToCombination(const UPA_ShopItem* Ingredient, const UPA_ShopItem* CombinationItem)
{
