...
void UCAbilitySystemComponent::AuthApplyGameplayEffect(TSubclassOf<UGameplayEffect> GameplatEffect, int Level)
{
	if (GetOwner()&&GetOwner()->HasAuthority())
	{
		FGameplayEffectSpecHandle EffectSpecHandle= MakeOutgoingSpec(GameplatEffect,1,MakeEffectContext());
		ApplyGameplayEffectSpecToSelf(*EffectSpecHandle.Data.Get());
	}
...
void UCAbilitySystemComponent::HealthUpdated(const FOnAttributeChangeData& ChangeData)
{
	...
	{
		if (!HasMatchingGameplayTag(UCAbilitySystemStatics::GetHeathFullStatTag()))
		{
			AddLooseGameplayTag(UCAbilitySystemStatics::GetHeathFullStatTag());
		}
	}
	else
	{
		RemoveLooseGameplayTag(UCAbilitySystemStatics::GetHeathFullStatTag());
	}
	...
	if (ChangeData.NewValue <= 0 )
	{
		...
		if (!HasMatchingGameplayTag(UCAbilitySystemStatics::GetHeathEmptyStatTag()))
		{
			AddLooseGameplayTag(UCAbilitySystemStatics::GetHeathEmptyStatTag());
		...
		{
			RemoveLooseGameplayTag(UCAbilitySystemStatics::GetHeathEmptyStatTag());
		}
